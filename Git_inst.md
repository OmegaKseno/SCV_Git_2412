...
## 16. Загрузка изменений с локального на удаленный репазиторий
 ...
 ## 18. Создание pull request в чужей репазиторий.
 1. Что бы создать   *pull request* нужно клонировать репазиторий и создать отдельную ветку в нем. (все изменения которые мы хотим предлажить делать именно на этой ветке)
  
  2. После нужно отправить все изменения в удаленный репазиторий командой ` git push`. В лучае если такая ветка отсутствует в удаленном репазитории нужно воспользоваться командой `git push --set-upstream origin (название ветки) ` 
  3. Переходим на портал Github, Нажимаем на кнопку 
